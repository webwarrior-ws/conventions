...
export abstract class Plugins {
    ...
    public static headerMaxLengthWithSuggestions(
        ...
        const extraCharsAllowedBecauseOfARevertOrARevertOfARevert = "Revert \"".length

            // we add one because revert commits end with '"'
            + 1;

        const maxLineLengthAfterAccountingForRevertException = maxLineLength + extraCharsAllowedBecauseOfARevertOrARevertOfARevert;
        if (headerStr.endsWith("\"") && ((headerStr.startsWith("Revert \"") || headerStr.startsWith("Reapply \"")))) {
            theMaxLineLengthToCompareWith = maxLineLengthAfterAccountingForRevertException;
        ...
        if (!headerStr.startsWith("Merge ") && headerLength > theMaxLineLengthToCompareWith) {
            offence = true;
